...
Adaptée pour Sébastien Partouche — version session utilisateur
...
def calculate_volume(weight, height, kv, concentration_mg_ml, imc, calc_mode, charges, volume_cap):
    ...
    return min(volume, volume_cap), bsa
...
if not st.session_state["accepted_legal"] or st.session_state["user_id"] is None:
    ...
    st.markdown("Acceptez les mentions légales et créez ou sélectionnez votre identifiant utilisateur.")
    ...
    if st.button("Entrer dans la session"):
        if not accept: st.warning("Vous devez accepter les mentions légales.")
        else:
            chosen_id = new_user_id.strip() if new_user_id.strip() else user_id_input
            if not chosen_id: st.warning("Veuillez saisir ou sélectionner un identifiant.")
            else:
                st.session_state["accepted_legal"] = True
                st.session_state["user_id"] = chosen_id
                if chosen_id not in user_sessions: user_sessions[chosen_id] = {"programs": {}}
...
if os.path.exists(logo_path):
    try:
        ...
        st.markdown(f"""<div style="display:flex; align-items:center; gap:8px; background:#124F7A; padding:8px; border-radius:8px">
    ...
    except: st.title("Calculette de dose de produit de contraste — Oncologie adulte")
else: st.title("Calculette de dose de produit de contraste — Oncologie adulte")
...
with tab_params:
    ...
    program_choice = st.selectbox("Programme global", ["Aucun"] + list(libraries.get("programs", {}).keys()))
    ...
    if st.button("💾 Ajouter/Mise à jour programme (session)"):
        if new_prog_name.strip():
            user_sessions[st.session_state["user_id"]]["programs"][new_prog_name.strip()] = {k: config[k] for k in config}
            save_user_sessions(user_sessions)
...
with tab_patient:
    ...
    with col_birth: birth_year = st.select_slider("Année de naissance", options=list(range(current_year-120,current_year+1)), value=current_year-40)
    with col_prog:
        prog_choice_patient = st.selectbox(
            "Programme",
            ["Sélection d'un programme"] + list(user_sessions[st.session_state["user_id"]]["programs"].keys()),
            index=0
        )
    ...
    with col_mode_time:
        ...
        with col_times:
            base_time = config.get("portal_time",30.0) if injection_mode=="Portal" else config.get("arterial_time",25.0) if injection_mode=="Artériel" else st.number_input("Temps Intermédiaire (s)", value=config.get("intermediate_time",28.0), min_value=5.0,max_value=120.0,step=1.0)
    ...
    if config.get("simultaneous_enabled",False):
        ...
        current_conc=config.get("concentration_mg_ml",350)
    ...
    if time_adjusted: st.warning(f"⚠️ Temps d’injection ajusté à {injection_time:.1f}s pour respecter le débit maximal de {config.get('max_debit',6.0)} mL/s.")
