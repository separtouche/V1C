...
Version optimisée avec sessions utilisateurs isolées
"""
...
global_libraries = load_json_safe(LIB_FILE, {"programs":{}})
if "programs" not in global_libraries: global_libraries["programs"]={}

# ------------------------
# Mentions légales + session
# ------------------------
if not st.session_state["accepted_legal"]:
    st.header("⚠️ Mentions légales et identification")
    st.markdown("Avant utilisation, acceptez la mention légale et choisissez/créez votre identifiant de session. Résultats indicatifs à valider par un professionnel de santé.")
    
    # création / sélection session
    user_list = [f.split(".json")[0] for f in os.listdir(USER_LIBS_DIR) if f.endswith(".json")]
    user_selection = st.selectbox("Sélectionnez votre identifiant existant", [""]+user_list)
    new_user = st.text_input("Ou créez un nouvel identifiant")
    ...
    if st.button("Entrer dans la session"):
        if not accept:
            ...
            if new_user.strip():
                ...
                st.session_state["user_id"]=user_selection
            else:
                st.warning("Sélectionnez ou créez un identifiant.")
...
# ------------------------
# Charger bibliothèque utilisateur
# ------------------------
...
with tab_params:
    ...
    st.subheader("📚 Bibliothèque globale de programmes")
    prog_choice_global=st.selectbox("Programme global", ["Aucun"]+list(global_libraries.get("programs",{}).keys()))
    if prog_choice_global!="Aucun":
        prog_conf = global_libraries["programs"].get(prog_choice_global,{})
        for k,v in prog_conf.items(): config[k]=v
    
    # bibliothèque utilisateur
    st.subheader("📚 Vos programmes")
    new_prog_name = st.text_input("Nom du nouveau programme à sauvegarder")
    ...
    if user_libraries.get("programs"):
        sel_prog_user = st.selectbox("Sélectionner programme utilisateur", [""]+list(user_libraries["programs"].keys()))
        if sel_prog_user:
            prog_conf = user_libraries["programs"].get(sel_prog_user,{})
            for k,v in prog_conf.items(): config[k]=v
        # suppression
