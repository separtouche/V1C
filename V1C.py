...
default_config = {
    ...
    "auto_acquisition_by_age": False,   # default: NOT checked
    "max_debit": 6.0,
    "rincage_volume": 35.0,
    "rincage_delta_debit": 0.5,
    "calc_mode": "Charge iodée",
    "simultaneous_enabled": False,     # default: NOT checked
...
# Ensure each user has expected keys
for uid, d in list(user_sessions.items()):
...
if not st.session_state["accepted_legal"] or st.session_state["user_id"] is None:
    ...
    st.caption("Si vous oubliez votre identifiant, utilisez 'Identifiant oublié ?' pour retrouver via email (si renseigné).")
...
with tab_params:
    ...
    # These are stored per-user and default to False (user asked)
    ...
    st.markdown("---")
    st.subheader("Paramètres techniques avancés")
...
with tab_patient:
    ...
    with col_w:
        ...
        # mode injection directly under weight
        cfg = get_user_cfg()
    ...
    with col_prog:
        ...
        prog_choice_patient = st.selectbox("Programme", ["Sélection d'un programme"] + list(user_programs.keys()), index=0, key="prog_patient")
    ...
    # --- Display the required info under Taille & Naissance
    cfg = get_user_cfg()
    ...
    # --- Under the "Sélectionnez un programme" area show times & concentration as requested
    st.markdown("---")
    st.markdown("**Informations temps & acquisition**")
    ...
    injection_rate, injection_time, time_adjusted = adjust_injection_rate(volume, float(cfg.get("portal_time",30.0) if injection_mode=="Portal" else cfg.get("arterial_time",25.0)), float(cfg.get("max_debit",6.0)))

    # Compute simultaneous dilution / nacl breakdown if enabled
    if cfg.get("simultaneous_enabled", False):
        ...
        if target > current_conc:
            st.warning(f"La concentration cible ({target:.0f}) est supérieure à la concentration du flacon ({current_conc:.0f}). On utilisera la concentration du flacon.")
            target = current_conc
    ...
    # ------------------------
    # Bottom two cards (contrast = green drop SVG; NaCl = blue drop SVG)
    # ------------------------
