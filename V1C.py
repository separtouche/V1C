...
with tab_params:
    ...
    if st.button("💾 Ajouter/Mise à jour programme"):
        if new_prog_name.strip():
            libraries["programs"][new_prog_name.strip()] = {k: config[k] for k in config if k not in ["simultaneous_enabled", "target_concentration", "check_creatinine"]}
            save_libraries(libraries)
