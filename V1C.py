...
# scanner en oncologie hépatique selon les recommandations.
# ============================================================
...
with tab_params:
    ...
    # --- Activation départ d'acquisition par âge ---
    config["auto_acquisition_by_age"] = st.checkbox(
        "Ajuster départ d'acquisition automatiquement selon l'âge",
    ...
    # Le reste des paramètres existants (simultaneous_enabled, concentration, charges, etc.) reste inchangé
    # ... (code inchangé de ton onglet Paramètres)
...
with tab_patient:
    ...
    # --- Checkbox foie stéatosé pour UH ---
    foie_steatose = st.checkbox("Le foie est stéatosé ?")
    uh_rate = 120 if foie_steatose else 110

    # ========== Calculs principaux et affichage résultats ==========
    # ... (reste du code inchangé)

# ===================== Onglet Tutoriel =====================
with tab_tutorial:
    st.title("📘 Tutoriel — Mode d'emploi et principes cliniques")
    st.markdown(f"- **Foie sain (objectif)** : ≈ **{uh_rate} UH** (unités Hounsfield) au pic de rehaussement.")
    # ... (reste du tutoriel inchangé)
